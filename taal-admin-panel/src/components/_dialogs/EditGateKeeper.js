...
const gatekeeperSchema = z.object({
  name: z.string().min(2, "Gatekeeper name is required"),
...
export default function EditGateKeeper({ gatekeepers, onSuccess }) {
  ...
  const {
    ...
    defaultValues: {
      ...
      phone: gatekeepers.phone || "",
      email: gatekeepers.email || "",
    },
  });
  ...
  const onSubmit = async (values) => {
    const { data, error } = await request({
      method: "PUT",
      url: "/superadmin/update-gatekeeper",
      authRequired: true,
      payload: {
        gatekeeperId: gatekeepers._id,
        ...values,
      },
    });

    if (!error) {
      showToast("success", data.message || "Gatekeeper updated successfully.");
      setOpen(false);
      onSuccess(data.data);
    } else {
      ...
      <DialogContent className="w-full max-w-sm lg:max-h-[85vh] max-h-[70vh] rounded-2xl shadow-2xl bg-white dark:bg-gray-950 flex flex-col overflow-hidden border border-gray-200 dark:border-gray-800">
        <DialogHeader className="p-6 border-b border-gray-200 dark:border-gray-800">
          ...
          <DialogDescription className="text-sm text-gray-600 dark:text-gray-400">
            Update Gatekeeper details below.
          </DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6 overflow-y-auto bg-gray-50 dark:bg-gray-900 flex-1">
          <InfoInput label="Gatekeeper Name" icon={Building2} name="name" register={register} error={errors?.name} placeholder="Enter Gatekeeper name" />
