...
import EditRentProduct from "@/components/_dialogs/EditRentProduct";
// import DeleteRentProduct from "@/components/_dialogs/DeleteRentProduct"; // Uncomment if you have a delete component
    ...
    const fetchProduct = async () => {
      try {
        const { data, error } = await getOneProduct({
          method: "GET",
          url: `/superadmin/get-rent-product-by-id/?productId=${productId}`,
          authRequired: true,
        });
  ...
  const handleToggleStatus = async (productId, currentStatus) => {
    ...
    try {
      const { data, error } = await updateProductStatus({
        method: "PUT",
        url: `/superadmin/change-rent-product-status`,
        authRequired: true,
    ...
    <AdminDashboardLayout>
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between gap-3">
          <h2 className="text-xl font-bold">Rent Product Details</h2>
          <div className="flex items-center gap-2">
            <Button variant="outline" onClick={() => router.back()}>Back</Button>
            {product && (
              <>
                <EditRentProduct product={product} onSuccess={() => window.location.reload()} />
                {/* <DeleteRentProduct productId={product._id} onSuccess={() => router.push("/admin/rent-products")} /> */}
              </>
            )}
          </div>
        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                  ...
                  <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-1">
                        <div className="text-sm text-muted-foreground">Rent Price Per Day</div>
                        <div className="text-sm">{product.currency} {fmt(product.rentPricePerDay)}</div>
                      </div>

                      <div className="space-y-1">
                        <div className="text-sm text-muted-foreground">Deposit</div>
                        <div className="text-sm">{product.currency} {fmt(product.deposit)}</div>
                      </div>
                      ...
                      return (
                        <div key={v._id || idx} className="rounded-2xl border p-3 space-y-3">
                          ...
                          <div className="flex items-baseline gap-2">
                            <div className="text-lg font-semibold">{product.currency} {fmt(effectivePrice)}</div>
                            {hasDiscount && (
                              <div className="text-sm line-through text-muted-foreground">
                                {product.currency} {fmt(v.price)}
                              </div>
                            )}
                          </div>
                    ...
                    {product.variants?.map((v, i) => {
                      ...
                      return (
                        <TableRow key={v._id || i}>
                          ...
                          <TableCell>{product.currency} {fmt(v.price)}</TableCell>
                          <TableCell>{v.discountPrice ? `${product.currency} ${fmt(v.discountPrice)}` : "—"}</TableCell>
                          <TableCell>{product.currency} {fmt(final)}</TableCell>
